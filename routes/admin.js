    ...
    if(erros.length > 0){
        ...
        Categoria.findOne({_id: req.body.id}).then((categoria) => {
            ...
            categoria.save().then(() => {
                ...
                req.flash("error_msg", "Hoiuve um erro interno ao salvar a edicao da categoria")
                res.redirect("/admin/categorias")
            })
        }).catch((err) => {
...
router.get("/postagens/add", (req, res) => {
    Categoria.find().lean(true).then((categorias) => { //Usar .lean() para evitar o erro de não ter acesso ao db.
...
router.post("/postagens/nova", (req, res) => {
    ...
    if(req.body.descricao.length < 10){
        erros.push({texto: "Descricao muito pequeno"})
    }
    ...
    if(req.body.conteudo.length < 20){
        erros.push({texto: "Conteudo muito pequeno, voce pode se esforçar mais!"})
    }
    ...
    if(erros.length > 0){
        ...
        new Postagem(novaPostagem).save().then(() => {
            ...
            req.flash("error_msg", "Houve um erro ao salvar a nova postagem")
            res.redirect("/admin/postagens")
        })
    }
})
